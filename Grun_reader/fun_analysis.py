...
def annual_average(df_monthly, file_name):
    ...
    book = load_workbook(filename_xlsx)  # load workbook
    ...
    plt.savefig(graph_file)  # save graph
    worksheet = writer.sheets['Annual_avg']  # calling respective worksheet
    graph = openpyxl.drawing.image.Image(graph_file)  # save graph as openpyxl object
    graph.anchor = 'C1'  # cell to add graph in
    worksheet.add_image(graph)  # add graph in respective worksheet
    writer.save()
    os.remove(graph_file)  # delete graph image from folder
...
def annual_maximum(df_monthly, file_name):
    ...
    book = load_workbook(filename_xlsx)
    writer = pd.ExcelWriter(filename_xlsx, engine='openpyxl')
    writer.book = book
    ...
    graph = openpyxl.drawing.image.Image(graph_file)
    graph.anchor = 'C1'
